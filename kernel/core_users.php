<?php 
	...
	if(isset($_POST['user_register']))
	{
		...
		if($user == NULL || $name == NULL || $pass == NULL || $pas2 == NULL || $emal == NULL || $habo == NULL)
		{
		...
		else
		{
			...
			else
			{
				...
				else
				{
					...
					$userAdd = MYSQL_::Bits("INSERT INTO users_news (user,name,pass,email,habbo,ip,day_reg) VALUES ('$user','$name','$pas4','$emal','$habo','$usip','$dreg') ");
	...
	if(isset($_POST['user_profile']))
	{
		...
		$Remal = userD($USERID,'email');
		$Rhabo = userD($USERID,'habbo');

		//Verificamos que no existan campos vacios
		if($user == NULL || $name == NULL || $emal == NULL || $habo == NULL)
		{
			Alertas('dangers','Lo sentimos pero no puedes dejar campos vacios');
		}
		else
		{
			...
			if($Ruser == $user)
			{
					...
					if($user == $Ruser AND $name == $Rname AND $emal == $Remal AND $habo == $Rhabo)
					{
